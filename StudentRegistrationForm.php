...
<html lang="en">
    ...
    <body>
       ...
       <?php
        ...
        session_start();
        if(isset( $_SESSION["studentErroMsg"])){
            $student = unserialize($_SESSION["studentErroMsg"]);
            $uregno=$student->getUserRegNo();
            $uid = $student->getUserID();
            $uname= $student->getUserName();
            $ugender=$student->getUserGender();
            $date=$student->getUserDob();
            $ucontact= $student->getUserContactno();
            $guardian=$student->getUserGuardian();
            $relationship=$student->getRelationship();
            $gcontact=$student->getGuardianConNo();
            $paddress=$student->getUserPermanentAddr();
            $urole=$student->getUserRole();
            $uemail= $student->getUserEmail();
            $errorMsg= $student->getErrMsg();
            echo "<h3> $errorMsg </h3>";
        } else {
            $uregno="";
            $uid = "";
    ...
    <div class="form-container">
        <form id="Register" method="post" action="StudentRegistrationProcessing.php" style="background:var(--bs-gray-300);">
            <br>
            ...
            <table class="center" border="2" style="font-family: 'Abhaya Libre', serif;font-weight: bold;">
                <thead>
                    <tr>
                       <th > <center>Please Provide Following Details :</center ></th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <th><input type="text" name="uRegNo" id="form_uRegNo" value="<?=$uregno?>" required=""/>
                    <span class="error_form" id="uRegNo_error_message"></span>
                        </th>
                    </tr>
                    ...
                    <tr>
                        <th>Gender</th>
                        <th><input type="radio" name="uGender" value="male"><label>&nbsp Male</label><br>
                        <input type="radio" name="uGender" value="female">
                        <label>&nbsp Female</label><br></th>
                    </tr>
                    ...
                    <tr>
                    <th>Guardian</th>
                    <th><input type="text" name="uGuardian" id="form_uGuardian" value="<?=$guardian?>" required=""/>
                    <span class="error_form" id="uGuardian_error_message"></span> 
                    </th>
                    </tr>
                    
                    <tr>
                    <th>Relationship</th>
                    <th><input type="text" name="uRelationship" id="form_uRelationship" value="<?=$relationship?>" required=""/>
                    <span class="error_form" id="uRelationship_error_message"></span>
                    </th>
                    </tr>
                    
                    <tr>
                    <th>Guardian Contact No</th>
                    <th><input type="text" name="uGconNo" id="form_uGconNo" value="<?=$gcontact?>" required=""/>
                    <span class="error_form" id="uGconNo_error_message"></span>
                    </th>
                    </tr>
                    
                    <tr>
                    <th>Permanent Address</th>
                    <th><input type="text" name="uPaddr" id="form_uPaddr" value="<?=$paddress?>" required=""/></th>
                    </tr>
                    ...
                    <tr>
                        ...
                        <th><select id="question" name="question" class="required">
         ...
         <script type="text/javascript">
             ...
             $(function(){
                ...
                $("#form_uGuardian").focusout(function(){
                    check_uGuardian();
                });
                $("#form_uRelationship").focusout(function(){
                    check_uRelationship();
                });
                $("#form_uGconNo").focusout(function(){
                    check_uGconNo();
                });
                ...
                function check_uRegNo() {
                       var pattern = /^[0-9]*$/;
                       var uRegNo= $("#form_uRegNo").val();
                       if(pattern.test(uRegNo) && uRegNo !== '') {
                             $("#uRegNo_error_message").hide();
                             $("#form_uRegNo").css
                             ("border_bottom","2px solid #34F458")
                       } else {
                             $("#uRegNo_error_message").html
                             ("Should contain only digits");
                             $("#uRegNo_error_message").show();
                             $("#form_uRegNo").css
                 ...
                 function check_uGuardian() {
                         var pattern = /^[a-zA-Z]*$/;
                         var uGuardian = $("#form_uGuardian").val();
                         if(pattern.test(uGuardian) && uGuardian !== '') {
                             $("#uGuardian_error_message").hide();
                             $("#form_uGuardian").css
                             ("border_bottom","2px solid #34F458")
                         } else {
                             $("#uGuardian_error_message").html
                             ("Should contain only characters");
                             $("#uGuardian_error_message").show();
                             $("#form_uGuardian").css
                             ("border-bottom","2px solid #F90A0A");
                             error_uGuardian=true;
                        }
                }
                 
                 function check_uRelationship() {
                         var pattern = /^[a-zA-Z]*$/;
                         var uRelationship = $("#form_uRelationship").val();
                         if(pattern.test(uRelationship) && uRelationship !== '') {
                             $("#uRelationship_error_message").hide();
                             $("#form_uRelationship").css
                             ("border_bottom","2px solid #34F458")
                         } else {
                             $("#uRelationship_error_message").html
                             ("Should contain only characters");
                             $("#uRelationship_error_message").show();
                             $("#form_uRelationship").css
                             ("border-bottom","2px solid #F90A0A");
                             error_uRelationship=true;
                        }
                }
                 
                 
                function check_uGconNo() {
                        var pattern = /^([0-9]{10})*$/;
                        var uGconNo= $("#form_uGconNo").val();
                        if(pattern.test(uGconNo) && uGconNo !== '') {
                            $("#uGconNo_error_message").hide();
                            $("#form_uGconNo").css
                            ("border_bottom","2px solid #34F458")
                        } else {
                            $("#uGconNo_error_message").html
                            ("Should contain 10 digits");
                            $("#uGconNo_error_message").show();
                            $("#form_uGconNo").css
                            ("border-bottom","2px solid #F90A0A");
                            error_uGconNo=true;
                        }
                ...
                function check_uPass() {
                    ...
                    $("#Register").submit(function(){
                        ...
                        error_uGuardian = false;
                        error_uRelationship = false;
                        error_uGconNo = false;
                        error_uRole=false;
                        error_uEmail = false;
                        error_uPass = false;
                    ...
                    if(error_uRegNo === false && error_uID === false && error_uName === false && error_uContact === false && error_uGuardian === false && error_uRelationship === false && error_uGconNo === false && error_uRole === false &&  error_uEmail === false && error_uPass === false ) 
                         {
